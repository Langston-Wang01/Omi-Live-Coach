...
def store_conversation_state(uid: str, session_id: str, state: dict):
    """Store conversation state for live sessions"""
    key = f'conversation_state:{uid}:{session_id}'
    r.set(key, str(state), ex=3600)  # Expire after 1 hour


def get_conversation_state(uid: str, session_id: str) -> dict:
    """Get conversation state for live sessions"""
    key = f'conversation_state:{uid}:{session_id}'
    state = r.get(key)
    if state:
        return eval(state.decode('utf-8'))
    return {}


def update_conversation_metrics(uid: str, session_id: str, metrics: dict):
    """Update conversation metrics for analytics"""
    key = f'conversation_metrics:{uid}:{session_id}'
    r.set(key, str(metrics), ex=3600)  # Expire after 1 hour
